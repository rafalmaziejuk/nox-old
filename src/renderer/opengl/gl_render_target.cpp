...
void GLRenderTargetBase::clear(const Vector4D<float> &color, uint8_t index) const {
    NOX_ASSERT(index > m_colorAttachmentsCount);
...
void GLRenderTargetBase::clear(float depth) const {
    NOX_ASSERT(m_depthStencilAttachmentsCount == 0u);
...
GLRenderTarget::GLRenderTarget(const RenderTargetDescriptor &descriptor) : m_resolution{descriptor.resolution} {
    ...
    if (m_colorAttachmentsCount > 0u) {
        for (uint32_t i = 0; i < m_colorAttachmentsCount; i++) {
            ...
            createColorAttachment(*colorAttachments[i].texture, attachmentPoint);
        }
    ...
    if (m_depthStencilAttachmentsCount > 0u) {
        ...
        for (uint32_t i = 0; i < m_depthStencilAttachmentsCount; i++) {
            createDepthStencilAttachment(depthStencilAttachments[i].format);
        }
    }
...
uint8_t GLRenderTarget::validateDepthStencilAttachments(const DepthStencilAttachmentsContainer &attachments) {
    ...
    for (const auto &attachment : attachments) {
        if (attachment.format == Format::NONE) {
            break;
        }

        auto formatDescriptor = FormatHelper::getFormatDescriptor(attachment.format);
...
void GLRenderTarget::createColorAttachment(const Texture &texture, uint32_t attachmentPoint) {
    const auto &glTexture = static_cast<const GLTexture &>(texture);
...
void GLRenderTarget::createDepthStencilAttachment(Format format) {
    ...
    textureDescriptor.size = {m_resolution.x(), m_resolution.y(), 0u};
    m_depthStencilAttachments.emplace_back(textureDescriptor);
