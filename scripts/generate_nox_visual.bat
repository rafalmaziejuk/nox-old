...
pushd %~dp0\%BUILD_DIR%
cmake .. -G%COMPILER% -DBUILD_SHARED_LIBS=%BUILD_SHARED% -DNOX_INSTALL=%NOX_INSTALL% -DNOX_BUILD_EXAMPLES=%NOX_BUILD_EXAMPLES% -DNOX_BUILD_UNIT_TESTS=%NOX_BUILD_UNIT_TESTS%
if %ERRORLEVEL% NEQ 0 (
    echo CMake failed
    goto :FAIL
)
